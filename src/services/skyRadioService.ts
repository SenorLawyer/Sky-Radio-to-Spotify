...
export class SkyRadioService {
  private readonly url: string = "https://www.skyradio.nl/";
  private lastSong: string = "";
  private puppeteerFailCount: number = 0;
  private readonly MAX_PUPPETEER_FAILS: number = 3;

  /**
   * Scrapes the Sky Radio website to get the currently playing song using Puppeteer
  ...
  private async getCurrentlyPlayingSongWithPuppeteer(): Promise<SongData | null> {
    ...
    try {
      ...
      await page.waitForSelector('div[data-testid="now playing-text-container"]', {
        timeout: 30000,
      });

      const songData = await page.evaluate(() => {
        const titleElement = document.querySelector(
          'div[data-testid="now playing-text-container"] p.mui-s3fzg1.e111ficp5'
        );
        const artistElement = document.querySelector(
          'div[data-testid="now playing-text-container"] div.mui-14c1r2h.e111ficp8'
        );
  ...
  private async getCurrentlyPlayingSongFallback(): Promise<SongData | null> {
    try {
      ...
      const titleSelector = 'div[data-testid="now playing-text-container"] p.mui-s3fzg1.e111ficp5';
